...
export function isDynamic(def) {
	...
	for (const [key, val] of Object.entries(def)) {
		...
		if (val !== null && typeof val === "object" && typeof /** @type {any} */ (val).$ref === "string") return true;
...
export function hasAnyIsland(def) {
	...
	if (Array.isArray(def.children)) return def.children.some(/** @param {any} c */ (c) => hasAnyIsland(c));
	return false;
}
...
export function getPathValue(base, path) {
	if (!path) return base;
	return path.split("/").reduce((/** @type {any} */ acc, /** @type {string} */ key) => (acc == null ? undefined : acc[key]), base);
}
		...
		for (const [prop, val] of Object.entries(def.style)) {
			if (prop.startsWith("@")) {
				...
				for (const [sel, nestedRules] of Object.entries(/** @type {Record<string, any>} */ (val))) {
					...
					rules.push(`@media ${query} { ${resolved} { ${toCSSText(/** @type {any} */ (nestedRules))} } }`);
				}
			} else if (
				prop.startsWith(":") ||
				prop.startsWith(".") ||
...
function _walkSrc(def, srcs) {
	...
	if (def.state) {
		for (const d of Object.values(def.state)) {
			if (d && typeof d === "object" && /** @type {any} */ (d).$prototype === "Function" && /** @type {any} */ (d).$src) {
