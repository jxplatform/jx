...
A JSONsx component is a single `.json` file. All state, computed values, and functions are declared in `state`. Simple components are fully self-describing — no sidecar file required:
...
`this` is never used in JSONsx-managed code. All component state is accessed via `state`.
...
JSONsx separates type definitions from runtime variables:
...
JSONsx uses `$ref` to express bindings between properties and declared state, following the JSON Reference convention:
...
Inline properties are applied directly to the element. Nested rules are emitted as a scoped `<style>` block using a generated `data-jsonsx` attribute selector.
...
> **Status: Implemented.** Runtime handles `timing: "server"` entries. Dev server provides `/__jsonsx_server__` proxy. Compiler emits Hono handler for production.
...
A JSONsx component whose root `tagName` contains a hyphen is a custom element definition:
...
Custom elements render to the light DOM (no Shadow DOM). Style scoping uses `data-jsonsx` attributes.
...
*JSONsx Specification v2.0.0-draft — subject to revision*
